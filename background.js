    ...
    try {
        ...
        if (data[CHECKPOINT_KEY]) {
            ...
            if (timeSinceSave < 3600000) {
                ...
                originalFilename = checkpoint.originalFilename || "downloaded_file";
...
async function mergeChunks() {
    ...
    const finalBlob = new Blob(blobs, { type: finalMimeType });
...
function openCenteredPopup() {
    if (isCreatingPopup) return; // Stop rapid-fire duplicate requests from Chrome startup

    // If we think a popup is already open, check if it actually still exists
    if (popupWindowId !== null) {
        chrome.windows.get(popupWindowId, (win) => {
            if (chrome.runtime.lastError || !win) {
                // Window was closed by the user, safe to make a new one
                popupWindowId = null;
                launchPopup();
            } else {
                // Window is already open! Just bring it to the front
...
function launchPopup() {
    ...
    chrome.windows.getLastFocused((win) => {
        // Handle edge case where Chrome just started and has no focused window yet
        const baseWin = win || { left: 0, top: 0, width: 800, height: 600 };
        const width = 420;
        const height = 450;
        const left = Math.max(0, Math.round(baseWin.left + (baseWin.width / 2) - (width / 2)));
        const top = Math.max(0, Math.round(baseWin.top + (baseWin.height / 2) - (height / 2)));
...
chrome.downloads.onCreated.addListener(async (downloadItem) => {
    // 1. Ignore data/blob URLs generated by our own mergeChunks function
    if (!downloadItem.url.startsWith("http")) return;

    // 2. Prevent infinite loops if we fallback to Chrome's native downloader
    if (downloadItem.byExtensionId === chrome.runtime.id) return;

    // 3. Set a safe fallback name just in case
    originalFilename = downloadItem.filename ? downloadItem.filename.split(/[\\/]/).pop() : "download";

    // 4. Cancel the browser's default download so we can intercept
    chrome.downloads.cancel(downloadItem.id);

    try {
        const response = await fetch(downloadItem.url, { method: "HEAD", credentials: "include" });
        const contentLength = response.headers.get("Content-Length");
        const contentDisposition = response.headers.get("Content-Disposition");
        const contentType = response.headers.get("Content-Type") || downloadItem.mime || "";

        // Save the file type so the merger knows what to do with it later
        if (contentType) {
            finalMimeType = contentType.split(';')[0];
        }

        // 5. Try to get the real filename from the server's headers
        if (contentDisposition && contentDisposition.includes('filename=')) {
            const matches = /filename[^;=\n]*=((['"]).*?\2|[^;\n]*)/.exec(contentDisposition);
            if (matches != null && matches[1]) {
                originalFilename = matches[1].replace(/['"]/g, ''); // Remove quotes
            }
        } 
        
        // 6. If it STILL has no extension, extract it from the URL
        if (!originalFilename.includes('.')) {
            if (finalMimeType.includes('pdf')) originalFilename += '.pdf';
            else if (finalMimeType.includes('zip')) originalFilename += '.zip';
            else if (finalMimeType.includes('mp4')) originalFilename += '.mp4';
            else if (finalMimeType.includes('jpeg') || finalMimeType.includes('jpg')) originalFilename += '.jpg';
            else if (finalMimeType.includes('png')) originalFilename += '.png';
            else if (finalMimeType.includes('msdownload') || finalMimeType.includes('exe')) originalFilename += '.exe';
            else {
                // Try to rip it from the URL one last time
                try {
                    const urlPath = new URL(downloadItem.url).pathname;
                    const urlFilename = urlPath.substring(urlPath.lastIndexOf('/') + 1);
                    if (urlFilename && urlFilename.includes('.')) {
                        originalFilename += '.' + urlFilename.split('.').pop();
                    }
                } catch(e) {}
            }
        }

        // 7. CRITICAL: Clean illegal OS characters out of the filename so the save doesn't fail
        originalFilename = originalFilename.replace(/[<>:"/\\|?*]+/g, '_');

        // 8. Start the actual multithreaded download
        if (contentLength) {
            globalTotalBytes = parseInt(contentLength);
            isDownloading = true; 
            openCenteredPopup();
            initDownload(downloadItem.url, globalTotalBytes);
        } else if (downloadItem.totalBytes > 0) {
            globalTotalBytes = downloadItem.totalBytes;
            isDownloading = true;
            openCenteredPopup();
            initDownload(downloadItem.url, downloadItem.totalBytes);
        } else {
            console.log("[System] Unknown file size, falling back to Chrome.");
            chrome.downloads.download({ url: downloadItem.url });
        }
    } catch (err) {
        console.warn("[System] Fetch error or secure file, falling back to Chrome.", err);
        // If the server blocked our initial request, it will block the workers too.
        // Don't try to multithread it—just let Chrome handle it natively!
        chrome.downloads.download({ url: downloadItem.url });
    }
});
